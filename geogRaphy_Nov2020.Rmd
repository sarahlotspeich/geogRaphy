...
```{r, echo = F, cache = TRUE, warning = F, message = F, fig.width = 10, fig.height = 7}
options(digits=14)
ny <- read.csv("https://download.data.world/s/elzuv3txcj2hcxw7im2v7b4rlo3alj", stringsAsFactors = F) %>% 
  dplyr::mutate(long = NA, lat = NA)
for (r in 1:nrow(ny)) {
  geo_r <- ny$the_geom[r] %>% str_split(pattern = " ")
  long_r <- as.numeric(sub(pattern = "\\(", replacement = "", x = geo_r[[1]][2]))
  lat_r <- as.numeric(sub(pattern = "\\)", replacement = "", x = geo_r[[1]][3]))
  ny$long[r] <- long_r
  ny$lat[r] <- lat_r
}
...
ny %<>% dplyr::mutate(aline = factor(grepl("A", line), labels = c("No", "Yes"), levels = c(FALSE, TRUE)))
...
ny %>% dplyr::select(line) %>% unique() -> lines
lines$num <- sapply(X = str_split(string = lines$line, pattern = "-"), FUN = length)
ny %>% dplyr::left_join(lines) -> ny
...
---
# Example 1: Farmers Markets Directory and Geographic Data
...
```{r, echo = F, tidy = T}
options(width = 60)
farm <- read.csv("https://download.data.world/s/i2iwvf5bjnacid3s4m5uxodoepaaio", stringsAsFactors = F)
farm %>% select(fmid, street, city, county, state, zip) %>% head(3)
#farm %>% select(county, state, zip) %>% head(3)
```
...
```{r, tidy=TRUE}
p <- "https://download.data.world/s/i2iwvf5bjnacid3s4m5uxodoepaaio"
...
knitr::include_graphics('oak_ridge.png')
```
]
...
```{r, out.width='55%', fig.align='center', echo = F}
knitr::include_graphics('westminster.png')
```
]
...
Now we have a dataset of `r farm %>% nrow()` farmers markets in the US with their geocoded locations as .
...
It has been suggested that Haversine Formula is more robust than the Spherical Law of Cosines at very small distances (due to rounding). 
...
# Distance between Knoxville farmers markets

There are `r farm %>% dplyr::filter(state == "Tennessee", city == "Knoxville") %>% nrow()` registered farmers markets in Knoxville, TN. 

Let's use Haversine formula to find the distance between `fmid` `1019189` and `1019345`:
